 ...
 * 输入参数： sx,sy - 左上角坐标
 *            ex,ey - 右下角坐标
 *            color - 16位颜色数组(大端对齐,高位在前)
 * 输出参数： 无
 * 返 回 值： 无
 ***********************************************************************/
void Draw_ColorFill(uint32_t sx, uint32_t sy, uint32_t ex, uint32_t ey, uint16_t *color)
{
    ...
    for (i = 0; i < height; i++)
    {
        ...
        for(j=0;j<width;j++)
        {
            temp_buf = p[j];
            // 高低8位互换
            p[j] = ((temp_buf & 0x00FF) << 8) | ((temp_buf & 0xFF00) >> 8);
        }
        // 再批量发送
        LCD_WriteDatas((uint8_t *)p, width * 2);
        p += width;
    }
}
